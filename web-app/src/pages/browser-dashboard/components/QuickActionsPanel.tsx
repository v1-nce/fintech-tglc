...
const QuickActionsPanel = ({ onBrowseLoans, onMakePayment, onViewCredit }) => {
  const actions = [
    {
      id: 'browse',
      title: 'Browse Loans',
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
      {actions?.map((action) => (
        ...
        >
          <div className={`w-12 h-12 md:w-14 md:h-14 flex items-center justify-center rounded-lg ${action?.bg} group-hover:scale-110 transition-smooth flex-shrink-0`}>
          ...
          <Icon name="ArrowRight" size={18} className="text-muted-foreground group-hover:text-primary group-hover:translate-x-1 transition-smooth flex-shrink-0 mt-1" />
        </button>
      ))}
    </div>
  );
