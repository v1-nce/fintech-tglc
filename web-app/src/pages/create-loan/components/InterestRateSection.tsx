            ...
            {marketRates?.map((rate) => (
              <button
                ...
                className={`p-4 rounded-lg border transition-smooth text-left ${
                  parseFloat(interestRate) === rate?.rate
                    ? 'border-primary bg-primary/10' :'border-border bg-muted/30 hover:border-primary/50'
                }`}
              >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {parseFloat(interestRate) < 7 && "Your rate is below market average. This may attract more borrowers but yield lower returns."}
                  {parseFloat(interestRate) >= 7 && parseFloat(interestRate) <= 10 && "Your rate is competitive with current market conditions. Good balance of risk and reward."}
                  {parseFloat(interestRate) > 10 && "Your rate is above market average. Higher returns but may limit borrower interest."}
                </p>
              </div>
            </div>
          </div>
