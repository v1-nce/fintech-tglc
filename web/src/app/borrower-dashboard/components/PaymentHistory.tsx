      ...
      <div className="p-4 md:p-6 border-b border-border">
        <div className="flex items-center justify-between">
          ...
          <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg bg-primary/10">
            <AppIcon name="History" size={20} className="text-primary" />
          </div>
        </div>
      </div>
                ...
                <tr key={payment.id} className="hover:bg-muted/20 transition-smooth">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${statusConfig.bg} ${statusConfig.color}`}>
                      <AppIcon name={statusConfig.icon} size={14} />
                      {payment.status}
                    </span>
                  </td>
          ...
          return (
            <div key={payment.id} className="p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <span
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${statusConfig.bg} ${statusConfig.color} flex-shrink-0`}
                >
                  <AppIcon name={statusConfig.icon} size={14} />
                  {payment.status}
                </span>
              ...
              >
                ...
                <AppIcon name={isExpanded ? 'ChevronUp' : 'ChevronDown'} size={16} className="text-muted-foreground" />
              </button>
              {isExpanded && (
                <div className="mt-3 p-3 bg-muted/30 rounded-lg animate-slide-down">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="sm" fullWidth iconName="ExternalLink" iconPosition="left">
                      View
                    </Button>
                  </div>
                </div>
