...
export default function CreditScoreCard({ creditScore, rating, trend, onViewReport }: CreditScoreCardProps) {
    ...
    <div className="bg-card border border-border rounded-xl p-4 md:p-6 shadow-glow-md">
      <div className="flex items-start justify-between mb-4 md:mb-6">
        ...
        <div className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg ${getScoreBgColor(creditScore)}`}>
          <AppIcon name="Award" size={20} className={getScoreColor(creditScore)} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-4 md:mb-6 p-3 md:p-4 bg-muted/30 rounded-lg">
        <AppIcon name={getRatingIcon(rating)} size={18} className={getScoreColor(creditScore)} />
      ...
      <Button variant="outline" fullWidth iconName="FileText" iconPosition="left" onClick={onViewReport}>
        View Full Report
      </Button>
    </div>
  );
