...
export default function LoanAmountSection({ value, onChange, error, walletBalance }: LoanAmountSectionProps) {
  ...
  const handleMaxClick = () => {
    onChange(walletBalance.toString());
  };
        ...
        <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg border border-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-primary/10">
              <AppIcon name="Wallet" size={20} className="text-primary" />
            </div>
            <div>
        ...
        {value && parseFloat(value) > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
            ...
            <div className="p-3 md:p-4 bg-primary/10 rounded-lg border border-primary/20">
              ...
              <p className="text-sm md:text-base font-semibold text-primary">{formatCurrency(parseFloat(value) || 0)}</p>
            </div>
            <div className="p-3 md:p-4 bg-accent/10 rounded-lg border border-accent/20">
              ...
              <p className="text-sm md:text-base font-semibold text-accent">{formatCurrency(walletBalance - (parseFloat(value) || 0))}</p>
            </div>
          </div>
        )}
      </div>
